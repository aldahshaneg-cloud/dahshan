...
final class PilotAccountingWire
{
    ...
    /** معامل الساعة الإضافية — ساعة زيادة بتتحسب ساعة ونص (طلب 2026-09-10) */
    public const OVERTIME_FACTOR = 1.5;
    ...
    /**
     * 🔗 اتحاد فترات زمنية [بداية، نهاية] — المتداخل والملاصق بيتلموا في واحدة.
     *
     * ═══ الباج اللي اتعملت عشانه (بلاغ صاحب النظام 2026-09-11) ═══
     * الطيار ممكن يكون عليه **أكتر من وردية في نفس اليوم ومتداخلين**
     * (نقل بين فروع، أو وردية اتفتحت والقديمة ماتقفلتش). الحساب كان
     * بيجمع مدد الورديات جمع عادي:
     *
     *   • الساعات بتتضخّم — طيار موجود من ١٢:٥٥ لـ٠١:٠٠ (١٢ ساعة) طلع
     *     ١٦٫٠٧ ساعة لأن ورديتين اتجمعوا وهما متداخلين.
     *   • والإذن الواحد بيتخصم **مرتين** — لأنه بيتقصّ على كل وردية
     *     على حدة، فالإذن اللي جوه الاتنين بيتعدّ مرتين. اتشاف على
     *     الإنتاج: إذن ٠٠:١٨→٠٠:٢٧ مكرّر حرفيًا في نفس اليوم.
     *
     * الاتنين بيتحلّوا بنفس الحاجة: **اتحاد** الفترات مش مجموعها.
     *
     * بترجّع الفترات مرتّبة ومدموجة. الفاضية بترجّع فاضية.
     */
    public static function mergeIntervals(array $iv): array
    {
        $iv = array_values(array_filter($iv, static fn ($x): bool => is_array($x) && count($x) === 2 && $x[1] > $x[0]));
        if (! $iv) {
            return [];
        }
        usort($iv, static fn ($a, $b): int => $a[0] <=> $b[0]);
        $out = [array_shift($iv)];
        foreach ($iv as [$a, $b]) {
            $last = count($out) - 1;
            if ($a <= $out[$last][1]) {
                $out[$last][1] = max($out[$last][1], $b);   // متداخلة أو ملاصقة
            } else {
                $out[] = [$a, $b];
            }
        }
    ...
    /** مجموع مدد فترات متحدة — بالثواني */
    public static function spanSeconds(array $merged): int
    {
        $s = 0;
    ...
    public static function monthTotals(array $days, array $pilot, array $opts): array
    {
        ...
        /* ⏱ ساعات إضافية × ١.٥ (طلب صاحب النظام 2026-09-10):
           «الطيار أو أي موظف له عدد ساعات شغل — لو زاد عنها الساعة تتحسب
           بزيادة نص ساعة، يعني لو اشتغل ساعتين إضافي يبقى له ٣ ساعات».

           العتبة **يومية** مش شهرية، فالحساب لازم يبقى يوم بيوم: طيار
           عتبته ٨ وشغل ١٠ + ٦ = ١٦ ساعة على يومين، ياخد (٨ + ٢×١.٥) + ٦
           = ١٧ ساعة مدفوعة. لو اتحسبت على المجموع الشهري كان هياخد
           ١٦ بس (١٦ أقل من ٨×عدد الأيام) — يعني الإضافي يضيع.

           العتبة: ساعات الطيار المطلوبة (`required_daily_hours`) لو
           متحطّة، وإلا طول الوردية الافتراضي من الإعدادات (`shiftHours`).
           الموظفين مالهمش عمود خاص فبياخدوا الافتراضي.

           `hours` بيفضل الساعات **الفعلية** زي ما هي (ده اللي بيتعرض
           ويتراجع)، و`payHours` هي اللي الأجر بيتحسب منها. */
        $reqDaily = (float) ($pilot['required_daily_hours'] ?? 0) ?: $shiftHours;
        $payHours = 0.0;
        $otHours  = 0.0;
        foreach ($days as $d) {
            $h  = (float) $d['hours'];
            $ot = max(0.0, $h - $reqDaily);
            $otHours  += $ot;
            $payHours += ($h - $ot) + $ot * self::OVERTIME_FACTOR;
        }
        $t['requiredDailyHours'] = round($reqDaily, 2);
        $t['overtimeHours']      = round($otHours, 2);
        $t['payHours']           = round($payHours, 2);
        $t['hourRate']    = round($hourRate, 2);
        /* 🔴 الأجر من الساعات **المقرّبة زي ما بتتعرض** (بلاغ 2026-09-11:
           «الحسابات بتاعة الساعة مش مظبوطة»). كان بيتحسب من الرقم الخام قبل
           التقريب، فالشيت مايجمعش: ٢٥٥٫٥٣ × ٣٥ = ٨٩٤٣٫٥٥ والمكتوب ٨٩٤٣٫٣٨.
           فرق نص قرش، بس اللي بيراجع بالآلة الحاسبة بيلاقي رقمين مختلفين
           ويفتح شكوى. التقريب مرة واحدة، والباقي يتبعه. */
        $t['hourPay']     = round($t['payHours'] * $hourRate, 2);
    ...
    public static function filterTotals(array $t, array $keys): array
    {
        $map = [
            ...
            'hourPay'       => 'mon.hourPay',
            /* ⚠ الإضافي على نفس إذن أجر الساعات (2026-09-10): أي مفتاح
               مش في الخريطة دي بيعدّي الفلتر زي ما هو — فمن غير السطور
               دي موظف ممنوع من «أجر الساعات» كان هيشوف الساعات المدفوعة
               والإضافي والعتبة برضه، وياخد منهم الأجر بالطرح. */
            'payHours'           => 'mon.hourPay',
            'overtimeHours'      => 'mon.hourPay',
            'requiredDailyHours' => 'mon.hourPay',
