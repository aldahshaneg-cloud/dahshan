...
class PublicSiteController
{
     ...
     *
     * ⚠️ التحقق من التليفون هنا **أضيق من joinRequest**: الأصل بيقبل أي
     * 8→15 رقم (أرقام أرضي وأجنبية)، وده بيطلب **موبايل مصري صحيح**
     * `01[0125]` + 8 أرقام. الاتساع في joinRequest سلوك أصل مجمّد؛ المسار
     * الجديد مش مربوط بيه. التطبيع بيعدّي على `TrustWire::normalizePhone`
    ...
    public function contactMessage(Request $request): JsonResponse
    {
        ...
        if (preg_match('/^01[0125]\d{8}$/', $phone) !== 1) {
            throw new ApiException('اكتب رقم موبايل مصري صحيح — ١١ رقم يبدأ بـ 010 أو 011 أو 012 أو 015');
        }
