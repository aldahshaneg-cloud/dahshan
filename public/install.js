...
(function () {
   ...
   *
   * 🔴 2026-09-01: كل التطبيقات كان نطاقها `./` (متداخلة) فكروم كان بيمنع
   * التثبيت لو أي تطبيق دهشان تاني متثبّت — والحل وقتها كان إن كل manifest
   * يسمّي **كل** التطبيقات هنا عشان الحاجز يبان.
   * 🔴 2026-09-14 (بلاغ «تنزيل برنامج المحلات بيفتح برنامج العملاء»): التسمية
   * دي نفسها كانت بتخلّي كروم يعتبر تطبيق العملاء المتثبّت «نسخة» من تطبيق
   * المحلات فيفتحه بداله. الحل الجذري: لكل تطبيق `id` و`scope` = صفحته
   * بس (مفيش تداخل)، وrelated_applications بتسمّي **نفسه بس** — فاللي بيرجع
   * من getInstalledRelatedApps هو التطبيق الحالي لو متثبّت، والحاجز مابقاش
   * موجود أصلًا. `blockers` متسابة للتشخيص (بتفضل فاضية). */
