...
try {
    ...
    echo "\n══ 7) سعر الساعة الافتراضي بيسري على الموظف اللي سعره فاضي ══\n";
    /* كانت القاعدة العكس (موظف بسعر صفر أجره صفر) لحد ما صاحب النظام قرر
       2026-09-04: «خلي الموظفين ياخدوا السعر الافتراضي لو سعرهم فاضي».
       بنصفّر سعر الموظف ونحط افتراضي 999 — أجر ساعاته لازم يبقى 8 × 999،
       ولو رجع صفر يبقى القاعدة القديمة رجعت. */
    ...
    ok('🔴 موظف بسعر صفر بياخد الافتراضي: 8 ساعات × 999 = 7992',
        abs((float) ($r7['totals']['hourPay'] ?? -1) - 7992.0) < 0.01,
        'أجر الساعات: ' . ($r7['totals']['hourPay'] ?? '؟'));
    ok('والسعر الساري في الرد = 999 وسعره الخاص = 0',
        abs((float) ($r7['hourRate'] ?? -1) - 999.0) < 0.01 && abs((float) ($r7['ownHourRate'] ?? -1)) < 0.01,
        ($r7['hourRate'] ?? '؟') . ' / ' . ($r7['ownHourRate'] ?? '؟'));
