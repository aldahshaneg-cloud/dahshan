...
ok('  والرسالة بتشرح النتيجة',
   /هيمسح فرعه الحالي/.test(T));
...
ok('قايمة إضافة اللوحة بتشمل أصحاب الفرع الثابت',
   TC.includes('const inBranch   = pilots.filter(p => (' + gate + '));'));
ok('  والإضافة بتبعت الفرع الحاكم',
   TC.includes('branchId: (' + gate + ') || null'));
